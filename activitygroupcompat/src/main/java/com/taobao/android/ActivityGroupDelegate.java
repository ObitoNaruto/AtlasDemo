...
//http://blog.csdn.net/dfskhgalshgkajghljgh/article/details/52935455
/**
...
//                ? (Bundle) bundle.getBundle(STATES_KEY) : null;
        mLocalActivityManager.dispatchCreate(null);//containing group has been created后设置状态
...
//    }
    ...
    private void performLaunchChildActivity(ViewGroup container,String key,Intent intent ){
        ...
        mLocalActivityManager.startActivity(key,intent);//调起目标activity
        container.addView(
                mLocalActivityManager.getActivity(key)//将目标activity的decorView,填充到当前容器view内
                        .getWindow().getDecorView(),
                new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                        ViewGroup.LayoutParams.MATCH_PARENT));
    }
    ...
    public void execStartChildActivityInternal(ViewGroup container,String key, Intent intent){
        ...
        if (!StringUtils.equals(context.getPackageName(), packageName)) {//两个packageName相同
