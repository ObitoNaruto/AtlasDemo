...
package com.mobile.android.launcher.taobao.update;
...
public class Updater {
    ...
    public static void update(Context context) {
        ...
        try {
            ...
            if (!updateInfo.exists()) {//需要先推送patch文件到指定目录
                Log.e("update", "更新信息不存在，请先 执行 buildTpatch.sh");
                toast("更新信息不存在，请先 执行 buildTpatch.sh", context);
                return;
            }

            String jsonStr = new String(FileUtils.readFile(updateInfo));//从更新文件中读取数据转换成字符串
            UpdateInfo info = JSON.parseObject(jsonStr, UpdateInfo.class);//格式化
